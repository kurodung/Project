...
edf_file = "C:\\project\\signal\\DS-blink-control\\S3-B1-O2_INSIGHT2_49865_2023.11.01T17.20.58+07.00.edf"
...
# Apply a higher cutoff in the high-pass filter to remove baseline drift
raw1b.filter(1, 4, fir_design='firwin')  # For blink detection
raw2i.filter(12, 30, fir_design='firwin')  # For beta power analysis

# Get EEG data and sample rate
...
threshold = max_amplitude * 0.49

peaks, properties = find_peaks(time_range_signal, height=threshold, distance=int(sfreq * 0.35), width=int(sfreq * 0.1), prominence=max_amplitude * 0.2)
...
# Plotting the signal and detected blinks
...
# Apply a 12-30 Hz band-pass filter for the beta power analysis
beta_band_signal = raw2i.copy().filter(12, 30, fir_design='firwin')

# Get EEG data for imagination and no imagination periods
...
no_imagination_beta_power = np.sum(no_imagination_psd_beta, axis=1) * 10e+7
imagination_beta_power = np.sum(imagination_psd_beta, axis=1) * 10e+7
